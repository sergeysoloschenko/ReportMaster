...
Default mode for monthly reporting. Upload Outlook `.msg` files for the reporting month. ReportMaster parses email chains, extracts relevant attachment text, downloads public document links, removes duplicate content, and generates rows 4.1–4.5 of the approved report fragment:

1. Договор управления с гостиничным оператором Dusit
2. Техническое взаимодействие с гостиничным оператором Dusit
3. Работа с консультантами проекта
4. Взаимодействие с Dyer
5. Архитектурный надзор и пространственная координация

The monthly DOCX contains only the template table fragment: no title page, generation timestamp, thread list, participant list, or statistics page. The contract-reference column and responsible-person column come directly from the approved template and are not generated by the model.
...
CODEX_WORKDIR=/app
JOB_MAX_WORKERS=1
REQUIRE_LLM_FOR_MONTHLY=true
DOWNLOAD_LINKED_DOCUMENTS=true
LINK_DOWNLOAD_TIMEOUT_SECONDS=20
LINK_DOWNLOAD_MAX_SIZE_MB=25
LINK_DOWNLOAD_MAX_PER_MESSAGE=10
```
